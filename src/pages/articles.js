...
const MovinImg = ({ title, img, link }) => {
  ...
  function handleMose(e) {
    imgRef.current.style.display = "inline-block";
    x.set(0);
    y.set(-10);
  }

  function handleMosedata(e) {
  ...
  return (
    <Link
      href={link}
      target="_blank"
      onMouseMove={handleMose}
      onMouseLeave={handleMosedata}
    >
...
const Article = ({ img, title, date, link }) => {
  return (
    ...
    >
      ...
      <span className="text-primary font-semibold pl-4 dark:text-primaryDark sm:self-start sm:pl-0 xs:text-xs">
        {date}
      </span>
    </motion.li>
  );
